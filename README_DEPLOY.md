...
4. test 个人中心可以调整自动策略参数，并通过“详情”查看每个参数的含义、默认值和范围。
5. 默认策略：最大持仓 3 只，单票仓位 30%，涨幅 3%-18.8%，板块评分 84，个股评分 84，置信度 70，最高换手 25%，日线多头开启，每日只买一次；次日强势兑现阈值为浮盈 2%、开盘 1.5%、实时涨幅 3%，平盘确认阈值为浮盈不低于 -1%、开盘不低于 -1.2%。
6. 自动买入任务：交易日北京时间 14:45，对应 Cron `45 6 * * 1-5`。
7. 自动卖出任务：交易日北京时间 09:35，对应 Cron `35 1 * * 1-5`。
8. 策略只做模拟记录，不连接券商，不发送真实订单。
