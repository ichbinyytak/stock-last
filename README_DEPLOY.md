...
6. 默认选股策略：看板涨幅 3%-18.8%，换手 2%-25%，量比不低于 1，板块评分不低于 78，个股评分不低于 76，板块上涨广度不低于 45%，活跃家数不低于 3，日线多头默认关闭，避开近涨停开启，仅创业板候选开启，偏好 20cm 开启。
7. 可调前日K线过滤：默认要求候选股前一交易日不能涨停或跌停；如果前一交易日是阳线，阳线实体涨幅默认不能大于 5%。这些参数可在“选股策略参数”中调整。
8. 交易范围：默认只操作创业板。科创板、北交所、主板股票仍用于板块强度、20cm/30cm 前排、10cm 助攻等佐证，不进入买入候选。
9. 自动买入任务：交易日北京时间 14:45，对应 Cron `45 6 * * 1-5`。
10. 自动卖出任务：交易日北京时间 09:35，对应 Cron `35 1 * * 1-5`。
11. 策略只做模拟记录，不连接券商，不发送真实订单。
