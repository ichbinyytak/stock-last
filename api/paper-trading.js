...
const MAX_POSITION_VALUE = INITIAL_CAPITAL * 0.3;
...
function flattenCandidates(result, account) {
  ...
  return (result.boards || [])
    ...
    .filter(({ stock }) => Number(stock.change || 0) >= 3 && Number(stock.change || 0) <= 18.8)
    .filter(({ stock }) => stock.risk !== "高换手")
    .filter(({ stock }) => stock.action === "尾盘候选" || Number(stock.score || 0) >= 84)
    .filter(({ stock }) => stock.trend && stock.trend.bullish)
    .sort((a, b) => {
...
function buyQuantity(cash, price, slots) {
  const budget = Math.min(MAX_POSITION_VALUE, cash / Math.max(slots, 1));
...
async function buyLateDayCandidates(account) {
  ...
  const today = shanghaiParts().dateKey;
  if (account.lastBuyDate === today) {
    recordEvent(account, "SKIP_BUY", "今日已执行买入", "模拟盘每天只在尾盘窗口执行一次新开仓。");
    return 0;
  }
  const slots = Math.max(0, MAX_POSITIONS - positionRows(account).length);
  if (!slots) {
    recordEvent(account, "SKIP_BUY", "仓位已满", `模拟盘最多同时持有 ${MAX_POSITIONS} 只。`);
    return 0;
  }
  ...
  candidates.forEach(({ board, stock }, index) => {
    ...
    const quantity = buyQuantity(account.cash, price, slots - count);
...
async function handler(req, res) {
  ...
  try {
    ...
    let run = null;
    if (url.searchParams.get("run") === "1" || req.method === "POST") {
